...
{
    //err_print("try to find symbol");
    symbol_t *stmp = symtab->next;
    while (stmp) {
        if (stmp->name && strcmp(stmp->name, name) == 0)
	        return stmp;
        stmp = stmp->next;
    } 
...
int add_symbol(char *name,int64_t addr)
{
    ...
    symbol_t *temp = (symbol_t *)malloc(sizeof(symbol_t)); // free in finit
    memset(temp, 0, sizeof(symbol_t));
    temp->name = name;
    temp->addr = addr;
    ...
    symtab->next = temp;
    //err_print("add symbol: %s, addr: %ld",temp->name,temp->addr);
    return 0;
}
...
int add_reloc(char *name, bin_t *bin)
{
    ...
    reloc_t *temp = (reloc_t *)malloc(sizeof(reloc_t)); // free in finit
    memset(temp, 0, sizeof(reloc_t));
    temp->name = name;
    temp->y64bin = bin;
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    int len = 0;
    if(IS_LETTER(p)){ 
        while(IS_LETTER(p + len) || IS_NUM(p + len)) len++;

        /* allocate name and copy to it */
        char *n = *name;
        n = (char *)malloc(len); // free in finit
        memset(n, 0, len);
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    else if(strncmp(p,"-0x",3) == 0){
        p += 3;
        val = -strtoll(p,&next,16);
    }
    else {
        val = strtoll(p,&next,10);
    }

    if(val == 0 && p[0] != '0') return PARSE_ERR;
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    ...
    *ptr = p;
    ...
    else if(IS_LETTER(p)) result = parse_symbol(ptr,name);
    
    /* set 'ptr' and 'name' or 'value' */
    if(result == PARSE_ERR) err_print("Invalid Immediate");
    return result;
}
...
parse_t parse_data(char **ptr, char **name, long *value)
{
    ...
    *ptr = p;
    ...
    if(IS_DIGIT(p)) return parse_digit(ptr,value);

    /* if IS_LETTER, then parse the symbol */
    if(IS_LETTER(p)) return parse_symbol(ptr,name);
...
*           call SUM  #invoke SUM function */
    ...
    if(IS_LETTER(ptr) && strstr(ptr,":")){
        ...
        line->type = TYPE_INS;
        line->y64bin.bytes = 0;
        if(ifNeedDirect == TRUE) line->y64bin.addr = alignpos;
        else line->y64bin.addr = vmaddr;
        if(add_symbol(name,line->y64bin.addr) == -1){
    ...
    /* is an instruction ? */
    instr_t *ins;
    if(parse_instr(&ptr,&ins) == PARSE_ERR){
        if(line->type == TYPE_INS){
            SKIP_BLANK(ptr);
            if(IS_COMMENT(ptr) || IS_END(ptr)) return line->type;
            else
            {
                line->type = TYPE_ERR;
                return line->type;
    ...
    /* set type and y64bin */
    if(ifNeedDirect == TRUE && !(HIGH(ins->code) == I_DIRECTIVE && LOW(ins->code) != D_DATA)){
        vmaddr = alignpos;
        ifNeedDirect = FALSE;
    }
    line->type = TYPE_INS;
    ...
    switch (HIGH(ins->code)){
      ...
      case I_RRMOVQ: case I_ALU:
        if(parse_reg(&ptr,&regA) == PARSE_ERR || parse_delim(&ptr,',') == PARSE_ERR|| parse_reg(&ptr,&regB) == PARSE_ERR){
            line->type = TYPE_ERR; 
            return line->type;
        }
        else{
      ...
      case I_IRMOVQ: /* 3:0 F:regB imm */
        result = parse_imm(&ptr, &name, &val);
        if(result == PARSE_ERR || parse_delim(&ptr,',') == PARSE_ERR || parse_reg(&ptr,&regB) == PARSE_ERR){
            line->type = TYPE_ERR; 
            return line->type;
        }
        else if(result == PARSE_DIGIT){
            line->y64bin.codes[1] = HPACK(0xf,regB);
            for(int i = 0;i < 8;i++) line->y64bin.codes[i + 2] = val >> (i * 8);
        }
        else{
            line->y64bin.codes[1] = HPACK(0xf,regB);
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        if(parse_reg(&ptr,&regA) == PARSE_ERR || parse_delim(&ptr,',') == PARSE_ERR || parse_mem(&ptr,&val,&regB) == PARSE_ERR){
            line->type = TYPE_ERR; 
            return line->type;
        }
        else{
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        if(parse_mem(&ptr,&val,&regA) == PARSE_ERR || parse_delim(&ptr,',') == PARSE_ERR || parse_reg(&ptr,&regB) == PARSE_ERR){
            line->type = TYPE_ERR; 
            return line->type;
        }
        else{
      ...
      case I_JMP: case I_CALL:
        result = parse_data(&ptr,&name,&val);
        if(result == PARSE_ERR){
            ...
            err_print("Invalid DEST");
            return TYPE_ERR;
            //for(int i = 0;i < 8;i++) line->y64bin.codes[i + 1] = val >> (i * 8);
        }
        else{
            add_reloc(name,&(line->y64bin));
        }
      ...
      case I_DIRECTIVE:
        if(line->y64bin.bytes > 0){
            ...
            if(result == PARSE_DIGIT){
                for(int i = 0;i < line->y64bin.bytes;++i) line->y64bin.codes[i] = (val >> (i * 8)) & 0xff;                
            }
        ...
        else if(parse_digit(&ptr,&val) == PARSE_DIGIT){
            if(LOW(ins->code) == D_ALIGN && (val == 2 || val == 4 || val == 8)){
                alignpos = ((vmaddr / val) + !!(vmaddr % val)) * val;
                ifNeedDirect = TRUE;
                line->type = TYPE_DIRECT;
            }
            else if (LOW(ins->code) == D_POS){
                alignpos = val;
                ifNeedDirect = TRUE;
                line->type = TYPE_DIRECT;
            }
...
int relocate()
{
    ...
    while (rtmp) {
        ...
        int start = ((rtmp->y64bin->bytes == 9)? 1: 2);
        if(HIGH(rtmp->y64bin->codes[0]) == I_DIRECTIVE) start = 0;
        for(int i =0;i < 8;i++) rtmp->y64bin->codes[i + start] = (sym->addr) >> (i * 8);
...
int binfile(FILE *out)
{
    ...
    int size = vmaddr;
    byte_t *image = (byte_t *)malloc(size);
    memset(image, 0, size);

    line_t *ltmp = line_head;
    while (ltmp) {
        if(ltmp->type == TYPE_INS){
            // if(HIGH(ltmp->y64bin.codes[0]) == I_JMP){
            //     long tempv = 0;
            //     for(int i = 0;i < 8;++i) tempv = tempv | (ltmp->y64bin.codes[1+i] << 8*i);
            //     if(tempv >= size){
            //         err_print("Invalid dest");
            //         return -1;
            //     }
            ...
            for(int i = 0;i < ltmp->y64bin.bytes;++i)
                image[ltmp->y64bin.addr + i] = ltmp->y64bin.codes[i];
        }
        ltmp = ltmp->next;
    }

    /* binary write y64 code to output file (NOTE: see fwrite()) */
    return (fwrite(image,sizeof(byte_t),size,out) - size);
}
