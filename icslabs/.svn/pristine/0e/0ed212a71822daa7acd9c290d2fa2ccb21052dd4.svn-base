...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode)
    {
    case I_ALU: case I_IRMOVQ: case I_MRMOVQ: case I_RMMOVQ: case I_RRMOVQ: case I_POPQ: case I_PUSHQ:
        /* code */
    ...
    switch (icode)
    {
    case I_IRMOVQ: case I_MRMOVQ: case I_RMMOVQ:  case I_JMP: case I_CALL:
        /* code */
        if (!get_long_val(sim->m, next_pc, &immed)) {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        next_pc += 8;
    ...
    switch (icode) {
      ...
      case I_ALU: /* 6:x regA:regB */
        ...
        break;
      case I_JMP: /* 7:x imm */
        sim->pc = (cond_doit(sim->cc,ifun)?immed:next_pc);
        break;
      case I_CALL: /* 8:x imm */
